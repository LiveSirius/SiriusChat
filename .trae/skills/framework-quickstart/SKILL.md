...
21. `sirius_chat/core/prompt_builders.py`（prompt 组装与 LLM 生成 Mixin）
22. `sirius_chat/core/bg_tasks.py`（7 个后台任务 Mixin）
23. `sirius_chat/core/helpers.py`（技能集成、用户画像、token 记录 Mixin）
24. `sirius_chat/core/cognition.py`
25. `sirius_chat/core/response_assembler.py`
26. `sirius_chat/core/response_strategy.py`
...
- `EmotionalGroupChatEngine` 是 **v1.0 唯一引擎**，采用 Mixin 架构：`emotional_engine.py`（最终类）= `engine_core.py`（基类）+ `pipeline.py`（管线）+ `prompt_builders.py`（prompt）+ `bg_tasks.py`（后台任务）+ `helpers.py`（辅助）。
...
| `sirius_chat/core/` | 编排核心：EmotionalGroupChatEngine（Mixin 架构：engine_core + pipeline + prompt_builders + bg_tasks + helpers）、认知分析、响应策略、阈值引擎、节奏分析、事件总线、身份解析、表情包决策 | 不负责人格目录组织 |
...
| `sirius_chat/skills/` | SKILL 注册、依赖解析、执行、安全校验、遥测、数据存储；表情包子系统 `skills/sticker/`（向量检索、偏好管理、学习、反馈） | 不负责 provider 注册表 |
...
- **修改对话主流程**：优先检查 `sirius_chat/core/emotional_engine.py`、`core/engine_core.py`、`core/pipeline.py`、`core/prompt_builders.py`、`core/bg_tasks.py`、`core/helpers.py`、`core/response_assembler.py`、`core/cognition.py`、`core/response_strategy.py`。
