...
Plugin 可以通过 `plugin.json` 的 `permissions` 字段限制可调用者：
...
如果你的 Plugin 依赖第三方 Python 库，在 `plugin.json` 中声明：

```json
{
    "dependencies": ["requests>=2.28", "pillow>=10.0"],
    "resources": ["fonts/NotoSansSC.ttf"]
}
```

- `dependencies`：pip 包名及版本约束
- `resources`：打包到 Plugin 目录中的静态资源文件
...
`PluginResponse.render_mode` 可以动态覆写 `plugin.json` 或 `@command` 中的设定：
...
```
plugins/my_plugin/
├── plugin.json          # 必需：元数据声明
├── main.py              # 必需：PluginBase 子类
├── templates/           # 可选：模板文件
│   └── report.txt
└── resources/           # 可选：静态资源
    └── font.ttf
